...
function showHelp() {
  console.log(`
AUTO/MATE Static-frontend Development Stack
...
function startAgentServer(config) {
  ...
  spawnService(
    ...
    [
      ...agentServerCmd.args,
      // Pass the tools directory as a discrete CLI arg rather than relying on
      // OH_EXTRA_PYTHON_PATH. The agent-server splits that env var on ':' (like
      // PYTHONPATH), so project paths containing a colon (e.g. "AUTO:MATE") are
      // silently broken. A CLI argument is never split on ':'.
      "--extra-python-path",
      safeConfig.canvasToolsDir,
      "--host",
      "0.0.0.0",
      "--port",
...
function printBanner(config) {
  ...
  console.log(
    `${c.green}${c.bold}║${c.reset}  ${c.bold}AUTO/MATE Static-frontend Stack${c.reset}                          ${c.green}${c.bold}║${c.reset}`,
  );
  console.log(
    `${c.green}${c.bold}╠══════════════════════════════════════════════════════════════╣${c.reset}`,
  );
...
async function main() {
  ...
  console.log("");
  console.log(
    `${c.cyan}${c.bold}AUTO/MATE Static-frontend Development Stack${c.reset}`,
  );
  console.log("");
