...
const BENIGN_SOCKET_ERRORS = new Set([
  ...
  // Expected while an upstream service (e.g. the Vite dev server) is still
  // starting up. Logging every refused connection during the cold-start window
  // produces noise without actionable information.
  "ECONNREFUSED",
]);
...
export function createProxyHandlers({
  ...
  // Each in-flight proxied request adds a 'timeout' listener to the outgoing
  // socket. Node's default limit is 10; during startup (when the upstream target
  // isn't ready yet) many requests can pile up before the proxy timeout fires,
  // triggering a spurious MaxListenersExceededWarning. Raise the limit to a
  // value that comfortably exceeds the typical burst size.
  proxy.setMaxListeners(50);
  const metrics = {
