 ...
 * @param {string | null} [extraPythonPath] - Absolute path to add via
 *   --extra-python-path. Passed as a discrete CLI argument instead of through
 *   OH_EXTRA_PYTHON_PATH so that paths containing colons (e.g. a macOS project
 *   folder like /Users/x/Desktop/MY:PROJECT/tools) are not split on the ':'
 *   separator the agent-server uses when it reads the env var.
 * @returns {{ command: string, args: string[], source: string }}
 */
export function buildAgentServerCommand(env = process.env, extraPythonPath = null) {
