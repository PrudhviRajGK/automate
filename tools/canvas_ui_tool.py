...
class CanvasUIAction(Action):
    """Direct the AUTO/MATE frontend to perform a UI action."""
...
class CanvasUIExecutor(ToolExecutor[CanvasUIAction, CanvasUIObservation]):
    def __call__(
        ...
        return CanvasUIObservation.from_text(
            f"UI command '{action.command}' dispatched to the AUTO/MATE frontend."
        )


_CANVAS_UI_DESCRIPTION = """The user is interacting with you inside AUTO/MATE — a web UI with a chat panel on the left and a tabbed right-side panel (files, terminal, browser, vscode, planner, tasklist). This tool lets you drive that right-side panel so the user sees what you just produced.
...
* You browsed to a URL the user should see →
    ...
    browser interaction so AUTO/MATE has a screenshot to display, then call
...
class CanvasUITool(ToolDefinition[CanvasUIAction, CanvasUIObservation]):
    """Tool for controlling the AUTO/MATE UI from the agent."""
