/**
 * Electron Main Process — AUTO/MATE Desktop
 *
 * Starts the full AUTO/MATE stack (agent-server + automation via uvx,
...
app.whenReady().then(async () => {
  ...
  if (!uvxAvailable()) {
    dialog.showErrorBox(
      ...
      app.isPackaged
        ? "The bundled uv binary could not be found. Please reinstall AUTO/MATE."
  ...
  try {
    ...
    const errorTail = recentServiceErrors.length
      ...
      : "";
    dialog.showErrorBox("AUTO/MATE failed to start", summary + errorTail);
    app.quit();
  }
});
