...
describe("ChatInputLlmProfilePicker", () => {
  ...
  it("labels a free AUTO/MATE route in the profile menu", () => {
    useChatInputLlmProfileStateMock.mockReturnValue(
      state({
        profiles: [
          {
    ...
    expect(
      screen.getByText("AUTO/MATE DeepSeek V4 Flash (free)"),
    ).toBeInTheDocument();
  });

  it("labels a free AUTO/MATE route in the read-only profile menu", () => {
    useChatInputLlmProfileStateMock.mockReturnValue(
      state({
    ...
    expect(
      screen.getByTestId("chat-input-llm-profile-current"),
    ).toHaveTextContent("AUTO/MATE DeepSeek V4 Flash (free)");
  });
