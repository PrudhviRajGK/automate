...
> [!WARNING]
> This Helm chart is experimental. AUTO/MATE is an unauthenticated, single-tenant application. This helm chart doesn't address scale or zero-downtime deployments.

## When to use this

This chart turns a Kubernetes cluster into a shared, always-on AUTO/MATE
...
- **An internal "vibecoding" platform.** With [RBAC enabled](#rbac), give the
  ...
  AUTO/MATE UI can build and ship code into the cluster — and save it to
...
AUTO/MATE is an **unauthenticated, single-tenant** application. This chart
...
in. There is no built-in authentication in AUTO/MATE, so treat the release
