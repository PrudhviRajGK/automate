...
async function launchCloudChild(
  ...
  if (!backend) {
    return failure(
      "No AUTO/MATE Cloud backend is connected in AUTO/MATE.",
      'Ask the user to connect AUTO/MATE Cloud from the backend picker, then call this tool again — or relaunch now with target="local".',
    );
  }
  ...
  return {
    ...
    parent_link: false,
    parent_link_note:
      "This conversation runs on the local agent server, so AUTO/MATE Cloud has no parent to link the child to.",
  };
}
