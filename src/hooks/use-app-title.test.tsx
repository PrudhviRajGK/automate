...
describe("useAppTitle", () => {
  ...
  it("returns the OSS app title outside conversations", async () => {
    ...
    await waitFor(() => expect(result.current).toBe("AUTO/MATE"));
  });
  ...
  it("returns the conversation title with the OSS app name", async () => {
    ...
    await waitFor(() =>
      expect(result.current).toBe("My Conversation | AUTO/MATE"),
    );
  });
  ...
  it.each([
    ...
    async (status, emoji) => {
      ...
      await waitFor(() =>
        expect(result.current).toBe(`${emoji} My Conversation | AUTO/MATE`),
      );
    },
  );
