...
| Repository | Owns |
|---|---|
| [`OpenHands/OpenHands`](https://github.com/OpenHands/OpenHands) | AUTO/MATE frontend, user-facing control center, backend selection, and local-stack orchestration. |
...
- The launcher → backend → frontend → suffix plumbing is:
  ...
  - `scripts/ingress.mjs` and `scripts/static-server.mjs` proxy the real agent-server `/server_info` response and append `runtime_services` when configured. This keeps version/tool compatibility fields authoritative from the SDK while letting the AUTO/MATE stack advertise automation/frontend/ingress topology.
