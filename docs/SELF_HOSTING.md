# Self-Hosting AUTO/MATE on a Virtual Machine

This guide walks through running AUTO/MATE on a virtual machine (VM) so you
can reach it from anywhere via a browser.

> [!WARNING]
> AUTO/MATE drives an agent that can read and write the filesystem of the
...
3. **Run AUTO/MATE** — generate a key with `openssl rand -base64 32`, then `export LOCAL_BACKEND_API_KEY=<key>` and `npx @openhands/agent-canvas --public`
4. **(Optional) Get a domain** — point a domain at the machine with nginx + Let's Encrypt for TLS
5. **(Optional) Connect locally** — add the remote as a backend in your local AUTO/MATE
...
Start AUTO/MATE in public mode:
...
`LOCAL_BACKEND_API_KEY`, and confirm that you land in AUTO/MATE.

## 5. (Optional) Connect your local AUTO/MATE to the remote machine

If you already run AUTO/MATE locally, you can register the remote machine
as an additional backend and switch between local and remote from the UI.

1. In your local AUTO/MATE, open **Manage backends** → **Add a backend**:
