...
| AUTO/MATE / Agent Server | DefenseClaw equivalent |
|---|---|
...
| Component | Version |
|---|---|
| AUTO/MATE / Agent Server | Current `main` |
| Python | 3.10+ |
...
Follow the standard [AUTO/MATE quickstart](../README.md). The integration steps below assume the Agent Server is reachable at `http://localhost:18000`.
...
**Configure the LLM base URL in AUTO/MATE:**

Open the AUTO/MATE settings panel → select your active backend → under **LLM settings**, set **Base URL** to:
...
**Workflow recommendation:** Add `defenseclaw skill scan <skill-dir>` as a pre-commit or CI step in repositories that ship skills for AUTO/MATE.
...
The following integrations would require changes to AUTO/MATE, the Agent Server, or DefenseClaw, but would significantly deepen the security posture.
...
### 4. AUTO/MATE security dashboard

A dedicated panel in the AUTO/MATE UI that queries DefenseClaw's gateway REST API (`GET /alerts`, `GET /enforce/blocked`) would surface guardrail findings inline with the conversation view — correlating blocked prompts or tool calls with the agent turn that triggered them.
...
- [AUTO/MATE Self-Hosting](./SELF_HOSTING.md)
