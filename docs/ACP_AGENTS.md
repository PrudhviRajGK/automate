...
AUTO/MATE can drive your conversations with the built-in **OpenHands** agent or
...
AUTO/MATE calling an LLM directly, the Agent Server spawns the agent's own CLI
as a subprocess and relays each turn to it. The external agent manages its own
LLM, tools, and execution; AUTO/MATE sends messages and renders what comes
back.

```mermaid
flowchart LR
    canvas["AUTO/MATE<br/>(this UI)"]
    server["Agent Server"]
...
The Agent Server owns the subprocess and the credentials; AUTO/MATE only
...
2. **Check backend** — confirms AUTO/MATE can reach the Agent Server.
